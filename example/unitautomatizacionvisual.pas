...
const
  ...
  COLOR_CONDICION = $00C8D7FF; // salm贸n claro
  ...
  // los botones de editar y borrar dentro del 谩rea visible.
...
begin
  case Tipo of
    ...
    tbvRailComValido:
      Result := Format('RailCom v谩lido %d', [Direccion]);
...
begin
  ...
  R := MessageDlg('Guardar reglas',
    'Se han modificado las reglas. 驴Quieres guardar los cambios?',
...
begin
  ...
  LblAcciones.Caption := 'EJECUCI脫N DE LA REGLA';
  ...
  LblDescripcion.Caption := 'Descripci贸n';
  ...
  LblDireccion.Caption := 'Direcci贸n / sensor';
...
begin
  ...
  LblTituloPaletaCond.Caption := 'CONDICI脫N';
  ...
  CrearBotonPaleta(ScrollCondicion, 'RailCom v谩lido', tbvRailComValido, ubvCondicion, y, COLOR_CONDICION); Inc(y, 45);
  CrearBotonPaleta(ScrollCondicion, 'RailCom direcci贸n', tbvRailComDir, ubvCondicion, y, COLOR_CONDICION); Inc(y, 45);
  ...
  LblTituloPaletaAcc.Caption := 'EJECUCI脫N';
...
begin
  ...
  if (not Result) and AAvisar then
    MessageDlg('Edici贸n bloqueada',
      'El motor de automatismos est谩 en ejecuci贸n. Pare el motor antes de modificar reglas o grupos.',
      mtWarning, [mbOK], 0);
end;
...
begin
  ...
  // Ancho fijo y compacto. No depende del tama帽o del formulario ni del
  // ScrollBox, as铆 los botones de edici贸n y borrado siempre quedan visibles.
...
begin
  if not EdicionReglasPermitida(True) then Exit;
  G := TGrupoVisual(TPanel(Sender).Tag);
  if MessageDlg('Borrar grupo', '驴Borrar el grupo "' + G.Nombre + '"?',
...
begin
  ...
  R := TReglaVisual(TPanel(Sender).Tag);
  if MessageDlg('Borrar regla', '驴Borrar la regla "' + R.Nombre + '"?',
    mtConfirmation, [mbYes, mbNo], 0) <> mrYes then Exit;
...
begin
  ...
  BloqueSeleccionado := B;
  S := B.Descripcion;
  if InputQuery('Descripci贸n del bloque',
    'Descripci贸n adicional para la tarjeta. El nombre fijo del bloque no se modifica:', S) then
  begin
    B.Descripcion := S;
    MarcarModificado;
    MostrarPropiedadesBloque(B);
...
begin
  ...
  begin
    ...
    // Si viene de men煤 contextual como "Si X entonces", no se crea regla nueva:
    // se a帽ade X como condici贸n a la regla visual activa.
    if AccPart = '' then
    begin
      while CondPart <> '' do
      begin
