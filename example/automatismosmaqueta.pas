...
type
  ...
  public
    ...
    FuncState: Boolean;
    Grupo: string;
    DelayMs: Integer;      // compatibilidad: retardo mínimo
    DelayMaxMs: Integer;   // retardo máximo
...
begin
  ...
  // Primera evaluaci�n tras activar/cargar el grupo: programa el primer disparo, no dispara.
...
begin
  ...
  case C.Tipo of
    ...
    tcRailComLocoValido:
      begin
        // Condici�n de presencia f�sica: no compara la direcci�n DCC.
        // C.Present = True  -> TRUE si hay cualquier locomotora presente.
        // C.Present = False -> TRUE si no hay locomotora presente.
        //
        // Importante: para AUSENTE no exigimos HasCurrent. As� se comporta
        // como un sensor normal en OFF: si no consta ninguna presencia v�lida,
        // la condici�n de ausencia es verdadera. Esto evita que una regla del
        // tipo RailComLocoValido(1,AUSENTE) AND RailComLocoValido(2,AUSENTE)
        // quede bloqueada al arrancar o si el RailCom no ha emitido todav�a un
        // telegrama expl�cito de ausencia.
        Result := SensorEnRango(C.Sensor) and
                  ((FRailComInfo[C.Sensor].HasCurrent and
...
begin
  ...
  begin
    // El RailCom queda vac�o o sin DCC v�lido: no debe conservarse
    // la �ltima locomotora como si siguiera presente.
