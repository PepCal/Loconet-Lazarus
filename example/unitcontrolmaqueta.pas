...
uses
    ...
    IntfGraphics, FPImage, UnitAutomatismos, UnitAutomatizacionVisual;
...
{ =========================
  ...
  TForm_ControlMaqueta = class(TForm)
    ...
    BtnAutomatizacionVisual: TButton;
    ...
    procedure BtnAutomatizacionVisualClick(Sender: TObject);
...
begin
  ...
  S := Trim(TMenuItem(Sender).Hint);
  if S = '' then Exit;

  EsEjecucion := False;

  // El menú del mapa ya no inserta nunca en el editor de texto.
  // La marca COND| / EJEC| indica si el bloque entra como condición o ejecución.
  if Pos('COND|', S) = 1 then
  begin
    Delete(S, 1, 5);
    EsEjecucion := False;
  end
  else if Pos('EJEC|', S) = 1 then
  begin
    Delete(S, 1, 5);
    EsEjecucion := True;
  end;
  ...
  if EsEjecucion then
    FormAutomatizacionVisual.InsertarEjecucionDesdeMenuMapa(S)
  else
    FormAutomatizacionVisual.InsertarCondicionDesdeMenuMapa(S);
end;

procedure TForm_ControlMaqueta.CrearMenuContextualElementoRegla(E: TElementoMaqueta);

  function AddItem(AParent: TMenuItem; const ACaption, AInsert: string;
    AEsEjecucion: Boolean): TMenuItem;
  begin
    Result := TMenuItem.Create(FPopupElementoRegla);
    Result.Caption := ACaption;
    if AEsEjecucion then
      Result.Hint := 'EJEC|' + AInsert
    else
      Result.Hint := 'COND|' + AInsert;
...
begin
  ...
  case E.Tipo of
    etSensor:
      begin
        // Sensores: siempre condición. No hay rama AND ni ejecución.
        AddItem(Root,
          Format('Si Sensor(%d,ON)', [E.Addr]),
          Format('Sensor(%d,ON)', [E.Addr]),
          False);
        AddItem(Root,
          Format('Si Sensor(%d,OFF)', [E.Addr]),
          Format('Sensor(%d,OFF)', [E.Addr]),
          False);
      end;

    etRail:
      begin
        // RailCom: siempre condición. No hay rama AND ni ejecución.
        AddItem(Root,
          Format('Si RailCom(%d,Dcc,PRESENTE)', [E.Addr]),
          Format('RailCom(%d,Dcc,PRESENTE)', [E.Addr]),
          False);
        AddItem(Root,
          Format('Si RailCom(%d,Dcc,AUSENTE)', [E.Addr]),
          Format('RailCom(%d,Dcc,AUSENTE)', [E.Addr]),
          False);
        AddItem(Root,
          Format('Si RailComEstado(%d,PRESENTE)', [E.Addr]),
          Format('RailComEstado(%d,PRESENTE)', [E.Addr]),
          False);
        AddItem(Root,
          Format('Si RailComEstado(%d,AUSENTE)', [E.Addr]),
          Format('RailComEstado(%d,AUSENTE)', [E.Addr]),
          False);
        AddItem(Root,
          Format('Si RailComLocoValido(%d)', [E.Addr]),
          Format('RailComLocoValido(%d)', [E.Addr]),
          False);
        AddItem(Root,
          Format('Si RailComDir(%d,0)', [E.Addr]),
          Format('RailComDir(%d,0)', [E.Addr]),
          False);
        AddItem(Root,
        ...
        // Ejecuci�n: usar la locomotora detectada por este RailCom como direcci�n DCC.
        EjecMenu := AddSubMenu(Root, 'Ejecuci�n con locomotora RailCom');
        AddItem(EjecMenu,
    ...
    etSwitch:
      begin
        // Switch/desvío: puede insertarse como condición o como ejecución.
        CondMenu := AddSubMenu(Root, 'Condición');
        AddItem(CondMenu,
          Format('Si Switch(%d,ON)', [E.Addr]),
          Format('Switch(%d,ON)', [E.Addr]),
          False);
        AddItem(CondMenu,
          Format('Si Switch(%d,OFF)', [E.Addr]),
          Format('Switch(%d,OFF)', [E.Addr]),
          False);

        EjecMenu := AddSubMenu(Root, 'Ejecución');
        AddItem(EjecMenu,
          Format('Switch(%d,ON)', [E.Addr]),
          Format('Switch(%d,ON)', [E.Addr]),
          True);
    ...
    etGrafico:
      begin
        // Gráficos con dirección: se ofrecen como condición/ejecución genérica.
        if E.Addr > 0 then
        begin
          CondMenu := AddSubMenu(Root, 'Condición');
          AddItem(CondMenu,
            Format('Si Grafico(%d,ON)', [E.Addr]),
            Format('Grafico(%d,ON)', [E.Addr]),
            False);
          AddItem(CondMenu,
            Format('Si Grafico(%d,OFF)', [E.Addr]),
            Format('Grafico(%d,OFF)', [E.Addr]),
            False);
...
procedure TForm_ControlMaqueta.BtnAutomatizacionVisualClick(Sender: TObject);
begin
  if FormAutomatizacionVisual = nil then
    FormAutomatizacionVisual := TFormAutomatizacionVisual.Create(Application);

  FormAutomatizacionVisual.Control := ControlMaqueta1;
  FormAutomatizacionVisual.Show;
end;
